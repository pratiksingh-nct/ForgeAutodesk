...
router.get('/datamanagement', async (req, res) => {
    ...
    if (href === '#') {
        ...
        console.log('api 3 running')

    }
});

// The id parameter indicates the node that is being expanded: # means root node, so list hubs. After that it contains the href of the resource, so when expanding one hub the endpoint should return the projects for the hub. 
...
async function getHubs(oauthClient, credentials, res) {
    ...
    res.json(data.body.data.map((hub) => {
       ...
       console.log('get hub is working')
